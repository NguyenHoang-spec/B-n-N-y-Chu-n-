...
function App() {
  ...
  // 1d. Unified Memory Processing: Migration (384 -> 768) then Recovery (missing/broken)
  const processSessionMemory = async (sessionId: number) => {
    ...
    try {
        ...
        const toMigrateTurns = turns.filter(t => t.embedding && t.embedding.length === 384);
        const toMigrateWiki = wiki.filter(w => w.embedding && w.embedding.length === 384);
        
        const toRecoverTurns = turns.filter(t => !t.embedding || (t.embedding.length !== 768 && t.embedding.length !== 384));
        const toRecoverWiki = wiki.filter(w => !w.embedding || (w.embedding.length !== 768 && w.embedding.length !== 384));
        ...
        // 3. Limit processing to avoid hitting Gemini quota too hard (max 15 items per load)
        ...
        for (const item of itemsToProcess) {
            // Stop if too many errors (likely quota or network)
            if (consecutiveErrors >= 3) {
                setMemoryError("Hệ thống quá tải. Sẽ thử lại sau.");
                break;
            }
            ...
            try {
                ...
                if (text) {
                    if (item.isMigration) setMemoryStatus("Nâng cấp trí nhớ lên Gemini Cloud...");
                    else setMemoryStatus("Khôi phục ký ức bị thất lạc...");
